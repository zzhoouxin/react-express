        ...
        this.plugin('done', function (stats) {
            ...
            tplFiles.forEach(function (v) {
                if(/\.html/.test(v)){
                    ...
                    var dFileName = path.basename(v, 'html');
                    console.log('写入模板:) =========>', path.join(__dirname, toDir, dFileName + 'ejs'));
                    console.log('打包开始时间---->>>> ' + new Date());
                    var contentString = content.toString();
                    contentString=contentString.replace("{{duserCode}}","<%=duserCode%>").replace("{{interfaceUrl}}","<%=interfaceUrl%>")
                    ejs.delimiter = '$';
                    fs.writeFileSync(
                        path.join(__dirname, toDir, dFileName + 'ejs'),
                        ejs.render(contentString, assetsChunkFilter));
                }
            });
        });
    }
